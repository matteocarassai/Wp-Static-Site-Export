...
class WP_Static_Exporter {
	...
	// ... (Properties: temp_base_dir, temp_export_dir, etc.) ...
     ...
     * @param string|null $final_recipient_email The validated email address to use for the mailer config.
	 * @return array|false Array with 'url', 'filename', 'timestamp', and 'progress' on success, false on failure.
	 */
	public function run_export( $optimize_output = false, $use_fa_cdn = true, $convert_forms = false, $final_recipient_email = null ) { // Added final_recipient_email parameter
		...
		foreach ( $this->urls_to_process as $url ) {
			$this->process_url( $url, $optimize_output, $use_fa_cdn, $convert_forms ); // Pass convert_forms flag
		}
        ...
        // Pass recipient email to create_zip for dynamic config generation
	...
	private function process_url( $url, $optimize_output, $use_fa_cdn, $convert_forms ) {
        ...
        if ($doctype_pos === false) {
            $doctype_pos = stripos($html_content, '<html');
        }
        if ($doctype_pos !== false && $doctype_pos > 0) {
            $prefix_text = substr($html_content, 0, $doctype_pos);
            // Log only if the prefix seems substantial and not just whitespace
            if(trim($prefix_text) !== '') {
                $this->log_status( sprintf('Warning: Found and removed potential prefix text before DOCTYPE/HTML tag on %s. Prefix: %s...', $url, substr($prefix_text, 0, 100)) );
            }
        ...
        $elements_to_remove_queries = [ '//*[@id="comments"]', '//*[contains(@class, "comments-area")]', '//*[contains(@class, "comment-respond")]', '//*[contains(@class, "search-form")]', '//*[@role="search"]', '//*[@id="wpadminbar"]', '//script[contains(text(), "wp-emoji-release.min.js")]', '//link[@rel="https://api.w.org/"]', '//link[@rel="EditURI"]', '//link[@rel="wlwmanifest"]', '//link[@rel="shortlink"]', '//meta[@name="generator"]', ];
        ...
        if ( $optimize_output ) {
             ...
             // Add specific Fluent Forms script removal if optimizing AND converting forms (as they might interfere)
             if ($convert_forms) {
                 $optimization_queries[] = '//script[contains(@src, "fluentform")]';
                 $optimization_queries[] = '//script[contains(., "fluent_form_")]'; // Inline scripts
                 $optimization_queries[] = '//style[contains(., "fluentform")]'; // Fluent form styles
             }
        ...
        $this->log_status( sprintf( __( 'Discovering assets & rewriting links for %s', 'wp-static-exporter' ), $url ) );
                ...
                if ( $node->hasAttribute( $attribute ) ) {
                    ...
                    if ( ! empty( $asset_url ) ) {
                        ...
                        if ( $absolute_asset_url && strpos( $absolute_asset_url, $this->site_url ) === 0 ) {
                            ...
                            // Skip Fluent Forms JS if converting forms
                            ...
                            if ( ! $optimize_output || ! $is_css ) { $asset_relative_url = str_replace( $this->site_url, '', $absolute_asset_url ); $asset_save_path = $this->temp_export_dir . '/assets/' . ltrim($asset_relative_url, '/'); $relative_asset_path = $this->get_relative_path( $save_path, $asset_save_path ); $node->setAttribute( $attribute, $relative_asset_path ); }
                        }
                    }
                }
            }
        ...
        // If combining CSS, remove original internal <link> tags and add a single one
        if ( $optimize_output && !empty($internal_css_links_found_on_page) ) {
            ...
            if ($head && !$existing_combined_link) { $combined_css_save_path = $this->temp_export_dir . '/assets/combined-styles.css'; $relative_combined_path = $this->get_relative_path( $save_path, $combined_css_save_path ); $new_link = $dom->createElement('link'); $new_link->setAttribute('rel', 'stylesheet'); $new_link->setAttribute('id', 'wp-static-exporter-combined-styles'); $new_link->setAttribute('href', $relative_combined_path); $head->appendChild($new_link); $this->log_status( 'Added combined CSS link tag.' ); }
        }
        ...
        if ( $convert_forms ) {
            ...
            foreach ($forms as $form) {
                $form->setAttribute('action', $mailer_script_path_rel); $form->setAttribute('method', 'POST');
                $this->log_status('Updated form action and method.');
                // Remove WP/CF7/FluentForm hidden fields more broadly
                ...
                // Remove potential leftover Fluent Forms wrapper divs if needed (example)
                // $fluent_wrappers = $xpath->query('.//div[contains(@class, "fluentform")]', $form);
                // foreach ($fluent_wrappers as $wrapper) { ... remove ... }

                // Add form location hidden field
                $location_input = $dom->createElement('input'); $location_input->setAttribute('type', 'hidden'); $location_input->setAttribute('name', '_form_location'); $location_input->setAttribute('value', $url);
                $form->appendChild($location_input);
            }
        }

        // --- Rewrite Internal Links (<a> tags) ---
        $this->log_status( sprintf( __( 'Rewriting internal links for %s', 'wp-static-exporter' ), $url ) );
        $links = $xpath->query( '//a' );
        foreach ( $links as $link ) {
            if ( $link->hasAttribute( 'href' ) ) {
                $href = $link->getAttribute( 'href' ); $absolute_href = $this->make_absolute_url( $href, $url ); $processed_urls_slashed = array_map('trailingslashit', $this->urls_to_process);
                if ( $absolute_href && in_array( trailingslashit($absolute_href), $processed_urls_slashed ) ) { $target_relative_url = str_replace( $this->site_url, '', trailingslashit($absolute_href) ); $target_save_path = $this->temp_export_dir . '/' . $target_relative_url . 'index.html'; if ( empty($target_relative_url) || $target_relative_url === '/' ) { $target_save_path = $this->temp_export_dir . '/index.html'; } $relative_link_path = $this->get_relative_path( $save_path, $target_save_path ); $link->setAttribute( 'href', $relative_link_path ); }
                elseif ( $absolute_href && strpos( $absolute_href, $this->site_url ) === 0 ) { $link->setAttribute( 'href', '#' ); $this->log_status( sprintf( __( 'Warning: Rewriting internal link %s to # as target is not exported.', 'wp-static-exporter' ), $href ) ); }
            }
        }
        ...
        // --- Save Modified HTML ---
        if ( file_put_contents( $save_path, $modified_html_content ) === false ) { $this->log_status( sprintf( __( 'Error: Could not save HTML file %s', 'wp-static-exporter' ), $save_path ) ); }
        else { $this->log_status( sprintf( __( 'Saved HTML to: %s', 'wp-static-exporter' ), $save_path ) ); }
	}
	...
	private function download_assets( $optimize_output, $use_fa_cdn ) {
		...
		$this->downloaded_assets = []; $css_content_to_combine = ''; $processed_css_content = '';
		foreach ( $this->css_urls_to_combine as $asset_url ) {
             if ( in_array( $asset_url, $this->downloaded_assets ) ) continue;
             if ( $use_fa_cdn && (strpos($asset_url, 'fontawesome') !== false || strpos($asset_url, 'font-awesome') !== false) ) { $this->log_status( sprintf('Skipping CSS processing/combining for Font Awesome URL: %s', $asset_url) ); continue; }
             ...
             if ( ! is_wp_error( $response ) && wp_remote_retrieve_response_code( $response ) === 200 ) {
                 $css_content = wp_remote_retrieve_body( $response ); $asset_relative_url = str_replace( $this->site_url, '', $asset_url ); $asset_relative_url_path = parse_url( $asset_relative_url, PHP_URL_PATH );
                 if (!$asset_relative_url_path) { $this->log_status( sprintf( __( 'Warning: Could not parse path for CSS URL: %s. Skipping.', 'wp-static-exporter' ), $asset_url ) ); continue; }
                 ...
                 if ( $optimize_output ) { $css_content_to_combine .= "/* Source: " . esc_url($asset_url) . " */\n" . $processed_css_content . "\n\n"; $this->downloaded_assets[] = $asset_url; }
                 else { $asset_save_dir = dirname( $css_save_path ); if ( ! file_exists( $asset_save_dir ) ) { wp_mkdir_p( $asset_save_dir ); } if ( file_put_contents( $css_save_path, $processed_css_content ) === false ) { $this->log_status( sprintf( __( 'Error saving processed CSS %s to %s.', 'wp-static-exporter' ), $asset_url, $css_save_path ) ); } else { $this->log_status( sprintf( __( 'Saved processed CSS to: %s', 'wp-static-exporter' ), $css_save_path ) ); $this->downloaded_assets[] = $asset_url; } }
             } else { $this->log_status( sprintf( __( 'Error fetching CSS %s: %s', 'wp-static-exporter' ), $asset_url, is_wp_error( $response ) ? $response->get_error_message() : wp_remote_retrieve_response_message( $response ) ) ); }
        }
        if ( $optimize_output && ! empty( $css_content_to_combine ) ) {
            $combined_css_path = $this->temp_export_dir . '/assets/combined-styles.css';
            if ( ! file_exists( dirname( $combined_css_path ) ) ) { wp_mkdir_p( dirname( $combined_css_path ) ); }
             $this->log_status( sprintf( __( 'Saving combined CSS to: %s', 'wp-static-exporter' ), $combined_css_path ) );
            if ( file_put_contents( $combined_css_path, $css_content_to_combine ) === false ) { $this->log_status( sprintf( __( 'Error saving combined CSS to %s.', 'wp-static-exporter' ), $combined_css_path ) ); }
        }
    ...
    private function download_discovered_assets( $use_fa_cdn ) {
        ...
        $newly_downloaded_count = 0; $assets_to_process_pass = $this->assets_to_download; $processed_in_this_pass = [];
        while (!empty($assets_to_process_pass)) {
            $current_asset_url = array_shift($assets_to_process_pass);
            if ( in_array( $current_asset_url, $this->downloaded_assets ) || in_array($current_asset_url, $processed_in_this_pass) ) { continue; }
            $processed_in_this_pass[] = $current_asset_url;
            if ( $use_fa_cdn && (strpos($current_asset_url, '/webfonts/') !== false || strpos($current_asset_url, '/fontawesome/') !== false) ) { $path_info = pathinfo(parse_url($current_asset_url, PHP_URL_PATH)); if (isset($path_info['extension']) && in_array($path_info['extension'], ['woff', 'woff2', 'ttf', 'eot', 'svg'])) { $this->log_status( sprintf('Skipping Font Awesome font download due to CDN option: %s', $current_asset_url) ); continue; } }
            $asset_relative_url = str_replace( $this->site_url, '', $current_asset_url ); $asset_relative_url_path = parse_url( $asset_relative_url, PHP_URL_PATH );
            if (!$asset_relative_url_path) { $this->log_status( sprintf( __( 'Warning: Could not parse path for asset URL: %s. Skipping.', 'wp-static-exporter' ), $current_asset_url ) ); continue; }
            $asset_save_path = $this->temp_export_dir . '/assets/' . ltrim( $asset_relative_url_path, '/' ); $asset_save_dir = dirname( $asset_save_path );
            if ( ! file_exists( $asset_save_dir ) && ! wp_mkdir_p( $asset_save_dir ) ) { $this->log_status( sprintf( __( 'Error: Could not create asset directory %s. Skipping asset %s.', 'wp-static-exporter' ), $asset_save_dir, $current_asset_url ) ); continue; }
            ...
            if ( is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) !== 200 ) { $this->log_status( sprintf( __( 'Error downloading %s: %s', 'wp-static-exporter' ), $current_asset_url, is_wp_error( $response ) ? $response->get_error_message() : wp_remote_retrieve_response_message( $response ) ) ); continue; }
            $asset_content = wp_remote_retrieve_body( $response );
            if ( file_put_contents( $asset_save_path, $asset_content ) === false ) { $this->log_status( sprintf( __( 'Error saving asset %s to %s.', 'wp-static-exporter' ), $current_asset_url, $asset_save_path ) ); }
            else { $this->log_status( sprintf( __( 'Saved asset to: %s', 'wp-static-exporter' ), $asset_save_path ) ); $this->downloaded_assets[] = $current_asset_url; $newly_downloaded_count++; }
        }
    ...
    /**
     * Parses CSS content, queues assets, and rewrites paths.
     */
    private function process_css_content( $css_content, $original_css_url, $css_save_path, $use_fa_cdn ) {
        $this->log_status( sprintf( __( 'Processing CSS content from: %s', 'wp-static-exporter' ), $original_css_url ) );
        ...
        if ( ! empty( $matches[1] ) ) {
            ...
            foreach ( $found_urls as $original_path ) {
                ...
                if ( strpos( $path_in_css, 'data:' ) === 0 || preg_match( '/^(?:[a-z]+:)?\/\//i', $path_in_css ) ) { continue; }
                $absolute_asset_url = $this->make_absolute_url( $path_in_css, $original_css_url );
                if ( $absolute_asset_url && strpos( $absolute_asset_url, $this->site_url ) === 0 ) {
                    $is_fa_font = false;
                    if ( $use_fa_cdn && (strpos($absolute_asset_url, '/webfonts/') !== false || strpos($absolute_asset_url, '/fontawesome/') !== false) ) { $path_info = pathinfo(parse_url($absolute_asset_url, PHP_URL_PATH)); if (isset($path_info['extension']) && in_array($path_info['extension'], ['woff', 'woff2', 'ttf', 'eot', 'svg'])) { $is_fa_font = true; } }
                    if (!$is_fa_font) { if ( ! in_array( $absolute_asset_url, $this->assets_to_download ) && ! in_array( $absolute_asset_url, $this->downloaded_assets ) ) { $this->log_status( sprintf( __( 'Queueing asset found in CSS: %s', 'wp-static-exporter' ), $absolute_asset_url ) ); $this->assets_to_download[] = $absolute_asset_url; } }
                    else { $this->log_status( sprintf( __( 'Skipping queuing Font Awesome font found in CSS due to CDN option: %s', 'wp-static-exporter' ), $absolute_asset_url ) ); }
                    $asset_relative_url = str_replace( $this->site_url, '', $absolute_asset_url ); $asset_save_path = $this->temp_export_dir . '/assets/' . ltrim($asset_relative_url, '/');
                    $relative_path_in_css = $this->get_relative_path( $css_save_path, $asset_save_path ); $escaped_original_path = preg_quote( $original_path, '/' );
                    $css_content = preg_replace( '/url\(([\'"]?)' . $escaped_original_path . '([\'"]?)\)/i', 'url($1' . $relative_path_in_css . '$2)', $css_content );
                }
            }
        }
        return $css_content;
	 ...
	 */
	private function create_zip( $convert_forms, $final_recipient_email ) { // Added final_recipient_email
        $this->log_status( __( 'Creating ZIP archive...', 'wp-static-exporter' ) );
        if ( ! class_exists( 'ZipArchive' ) ) { $this->log_status( __( 'Error: ZipArchive class not found...', 'wp-static-exporter' ) ); return false; }
        $zip = new ZipArchive();
        if ( $zip->open( $this->zip_file_path, ZipArchive::CREATE | ZipArchive::OVERWRITE ) !== true ) { $this->log_status( sprintf( __( 'Error: Cannot open %s for writing.', 'wp-static-exporter' ), $this->zip_file_path ) ); return false; }

        // Create PHP Mailer files dynamically if forms were converted and email is valid
        if ( $convert_forms && ! empty( $final_recipient_email ) ) {
            ...
            if ( ! wp_mkdir_p( $mailer_dest_dir ) ) {
                ...
                // Generate mailer-config.php content
                $config_content = "<?php\n";
                ...
                // Add placeholders for other optional configs if needed
                // $config_content .= "// \$default_subject = 'Website Form Submission';\n";
                // $config_content .= "// \$default_from_address = 'noreply@yourdomain.com';\n";
                $config_content .= "?>\n";
                ...
                // Copy sendmail.php from plugin directory
        ...
        // Add exported site files from the temporary export directory recursively
        $source = realpath( $this->temp_export_dir );
        if ( is_dir( $source ) ) {
            $files = new RecursiveIteratorIterator( new RecursiveDirectoryIterator( $source, RecursiveDirectoryIterator::SKIP_DOTS ), RecursiveIteratorIterator::SELF_FIRST );
            foreach ( $files as $file ) {
                $file = realpath( $file ); $relativePath = substr( $file, strlen( $source ) + 1 );
                if ( is_dir( $file ) ) { $zip->addEmptyDir( $relativePath ); }
                elseif ( is_file( $file ) ) { $zip->addFile( $file, $relativePath ); }
            }
        } else { $this->log_status( sprintf( __( 'Error: Temporary export directory %s not found.', 'wp-static-exporter' ), $source ) ); $zip->close(); return false; }
        $zip_status = $zip->close();
        if ( $zip_status ) { $this->log_status( sprintf( __( 'ZIP archive created successfully: %s', 'wp-static-exporter' ), $this->zip_file_path ) ); return true; }
        else { $this->log_status( __( 'Error: Failed to close ZIP archive.', 'wp-static-exporter' ) ); return false; }
	}
	...
	private function cleanup( $remove_zip = true ) {
        ...
        if ( ! empty( $this->temp_export_dir ) && file_exists( $this->temp_export_dir ) ) { $this->remove_directory( $this->temp_export_dir ); $this->log_status( sprintf(__( 'Removed temporary directory: %s', 'wp-static-exporter'), $this->temp_export_dir) ); }
        if ( $remove_zip && ! empty( $this->zip_file_path ) && file_exists( $this->zip_file_path ) ) { unlink( $this->zip_file_path ); $this->log_status( sprintf(__( 'Removed ZIP file: %s', 'wp-static-exporter'), $this->zip_file_path) ); }
	}
    ...
    private function remove_directory( $dir ) {
        ...
        foreach ( $files as $file ) { ( is_dir( "$dir/$file" ) ) ? $this->remove_directory( "$dir/$file" ) : unlink( "$dir/$file" ); }
        return rmdir( $dir );
    }
	...
	private function make_absolute_url( $relative_url, $base_url ) {
		$relative_url = trim( $relative_url ); $base_url = trim( $base_url );
		if ( parse_url( $relative_url, PHP_URL_SCHEME ) != '' ) { return $relative_url; }
		if ( strpos( $relative_url, '//' ) === 0 ) { $base_scheme = parse_url( $base_url, PHP_URL_SCHEME ); return ($base_scheme ? $base_scheme : 'http') . ':' . $relative_url; }
		$base_parts = parse_url( $base_url );
		if ( ! $base_parts || !isset($base_parts['scheme']) || !isset($base_parts['host']) ) { if ( strpos( $relative_url, '/' ) === 0 ) { $site_parts = parse_url($this->site_url); if ($site_parts && isset($site_parts['scheme']) && isset($site_parts['host'])) { return $site_parts['scheme'] . '://' . $site_parts['host'] . $relative_url; } } return false; }
		if ( strpos( $relative_url, '/' ) === 0 ) { return $base_parts['scheme'] . '://' . $base_parts['host'] . (isset($base_parts['port']) ? ':' . $base_parts['port'] : '') . $relative_url; }
		$base_path = isset($base_parts['path']) ? $base_parts['path'] : '/';
		if ( substr( $base_path, -1 ) !== '/' ) { $base_path = dirname( $base_path ); }
		$base_path = rtrim( $base_path, '/' ) . '/';
		$absolute_url = $base_parts['scheme'] . '://' . $base_parts['host'] . (isset($base_parts['port']) ? ':' . $base_parts['port'] : '') . $base_path . $relative_url;
		$parts = explode( '/', $absolute_url ); $absolutes = [];
		foreach ( $parts as $part ) { if ( '.' == $part ) continue; if ( '..' == $part ) { array_pop( $absolutes ); } else { $absolutes[] = $part; } }
		$scheme_part = '';
		if (isset($absolutes[0]) && strpos($absolutes[0], ':') !== false) { $scheme_part = array_shift($absolutes) . '//' . array_shift($absolutes); }
		return $scheme_part . implode( '/', $absolutes );
	}
	...
	private function get_relative_path( $from_path, $to_path ) {
		...
		$from_path = str_replace( '\\', '/', $from_path ); $to_path = str_replace( '\\', '/', $to_path );
		$from_dir = pathinfo( $from_path, PATHINFO_DIRNAME ); $to_dir = pathinfo( $to_path, PATHINFO_DIRNAME );

        // Handle cases where paths might be identical or one is inside the other directly
        if ($from_dir === $to_dir) {
            // Removed logging
            return basename($to_path);
        }

		$from_parts = explode( '/', trim( $from_dir, '/' ) ); $to_parts = explode( '/', trim( $to_dir, '/' ) );

        // Remove empty elements caused by leading/trailing slashes after trim
        $from_parts = array_filter($from_parts);
        $to_parts = array_filter($to_parts);

		$common_prefix_length = 0; $max_length = min( count( $from_parts ), count( $to_parts ) );
		for ( $i = 0; $i < $max_length; $i++ ) { if ( $from_parts[$i] === $to_parts[$i] ) { $common_prefix_length++; } else { break; } }
        ...
        // Removed logging
		return empty($relative_path) ? basename($to_path) : $relative_path; // Fallback might still be needed?
	}
